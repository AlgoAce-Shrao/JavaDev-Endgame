...
💡 To implement the thread class , we can :
...
## Daemon Threads:-> These are background threads for which jvm doesn't depend or wait for...

there are two  types of threads:
1. User threads->the threads which we create and use to do some task
2. Daemon threads--> the background jobs which are performed like the garbage collector ...The funstion of hte user threads is not hampered by that of the daemon threads